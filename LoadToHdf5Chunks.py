...
def loadData(name):
    ...
    mFrame = 400
    hdf5_store = h5py.File("/home/atilocca/deceptionDetection/datasets/"+nameDS+"_datasetChunks.hdf5", "a")
    X = hdf5_store.create_dataset("X", (715,mFrame,width,height,channel),  maxshape=(None,mFrame,width,height,channel),compression="gzip")
    ...
    for i in dir:
        for filename in os.listdir(i):           
            ...
            count = 0
            if frameCount <= mFrame:
                vid = sequence.pad_sequences(vid, maxlen=mframe)
            #if frameCount % mframe != 0:
              #  mod = frameCount % mframe 
                #vid[-mod:]
            
            while count + mFrame <= frameCount:
                  chunk = vid[count:count + mFrame]
                  count = count + mFrame
                  reshape_frames = reshapeVid(chunk) 
                  y.append(label)
                  groups.append(subject)
                  ids.append(filename[-4]+'_'+str(count).zfill(3))                
                  X[videoCount,]= reshape_frames
